...
unsigned int JKeycodeToNative(jint key, jint location) {
	switch (key) {
		...
		case JK_SHIFT:
			if (location == JK_LOCATION_LEFT)	return XK_Shift_L;
			else								return XK_Shift_R;
		case JK_CONTROL:
			if (location == JK_LOCATION_LEFT)	return XK_Control_L;
			else								return XK_Control_R;
		case JK_ALT:
			if (location == JK_LOCATION_LEFT)	return XK_Alt_L;
		...
		case JK_META:
			if (location == JK_LOCATION_LEFT)	return XK_Meta_L;
		...
		case JK_WINDOWS:
			if (location == JK_LOCATION_LEFT)	return XK_Super_L;
