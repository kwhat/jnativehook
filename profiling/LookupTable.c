...
JKeyDatum keycode_table[256];

void initialize() {

	JKeyDatum keysym_table[] = {
	...
	int i;
	for (i = 0; i < sizeof(keysym_table) / sizeof(JKeyDatum); i++) {
		KeyCode keycode = XKeysymToKeycode(disp, keysym_table[i].rawcode);

		if (keycode != 0) {
			keycode_table[keycode] = keysym_table[i];
		}
	}
}

JKeyDatum lookup(unsigned int keysym) {
	return keycode_table[keysym];
}
